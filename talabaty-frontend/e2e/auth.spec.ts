...
/**
 * E2E tests for Sign up and Sign in.
 * Requires: backend running on port 8080 (e.g. run TalabatyApplication in IntelliJ),
 *           frontend will be started by Playwright if not already on port 3000.
 */
...
test.describe('Sign up', () => {
  ...
  test('full sign up flow redirects to app (select store or dashboard)', async ({ page }) => {
    ...
    await page.getByLabel(/account name/i).fill('E2E Test Account')

    await page.getByRole('button', { name: /get started/i }).click()

    // After signup we should leave the signup page and land in the app
    // (either Select a Store, or change-password, or dashboard)
...
test.describe('Sign in', () => {
  ...
  test('sign up then sign in with same user lands in app', async ({ page }) => {
    ...
    await page.getByLabel(/account name/i).fill('E2E Login Account')
    await page.getByRole('button', { name: /get started/i }).click()

    await expect(page).not.toHaveURL(/\/signup/)
    await expect(page).not.toHaveURL(/\/login/)

    // 2) Logout by clearing storage and go to login
