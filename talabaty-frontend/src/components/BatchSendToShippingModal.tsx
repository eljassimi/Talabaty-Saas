...
export default function BatchSendToShippingModal({
  orders,
  onClose,
  onSuccess,
}: BatchSendToShippingModalProps) {
  const [formData, setFormData] = useState<SendToShippingRequest>({
    cityId: '',
    note: '',
    nature: '',
    stock: 0, // Default: Pickup
    open: 1, // Default: Open
    fragile: 0,
    replace: 0, // Default: No
  })
  ...
  const [detectedCity, setDetectedCity] = useState<{ name: string; id: number } | null>(null)
  ...
  const detectCityFromOrders = () => {
    ...
    if (uniqueCities.length === 1) {
      // Single city - try to find its ID
      ...
      // Multiple cities - show warning with all cities
      setDetectedCity(null)
      setMultipleCities(uniqueCities)
      
      // Try to find the most common city or first match
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Ensure default values are set and properly formatted
      ...
      // If all succeeded, call onSuccess after a short delay
  ...
  return (
    <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div className="relative top-20 mx-auto p-5 border w-full max-w-2xl shadow-lg rounded-md bg-white max-h-[90vh] overflow-y-auto">
        ...
        {error && !result && (
          <div className="mb-3 bg-red-50 border border-red-200 text-red-700 px-3 py-2.5 rounded-lg">
            <div className="flex items-start">
              ...
              <div className="flex-1">
                ...
                {!formData.cityId && (
                  <p className="text-xs mt-2 font-medium">⚠ City ID is missing. Please ensure a city is auto-detected or selected.</p>
                )}
              </div>
            </div>
          </div>
            ...
            {result.successful && result.successful.length > 0 && (
              <div className="mb-3">
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {result.successful.map((item: any, idx: number) => (
                    <div key={idx} className="flex items-center text-xs text-gray-600 bg-green-50 px-2 py-1 rounded">
              ...
              <div>
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {result.failed.map((item: any, idx: number) => (
                    <div key={idx} className="flex items-start text-xs text-red-600 bg-red-50 px-2 py-1 rounded">
        ...
        {!result && (
          <form onSubmit={handleSubmit} className="space-y-3">
                      ...
                      {multipleCities.map((city, idx) => {
                        ...
                        return (
                          <li key={idx}>
                            ...
                            {cityId ? (
                              <span className="text-green-700 ml-1">✓ ID: {cityId}</span>
                            ) : (
                              <span className="text-red-700 ml-1">⚠ Not found</span>
                            )}
                          </li>
                        )
                      })}
            ...
            {!detectedCity && multipleCities.length === 0 && (
            ...
            {/* Show single city that's not found */}
            {multipleCities.length === 1 && !detectedCity && (
            ...
            {/* City selection - hidden but required for validation */}
            {loadingCities && (
              <div className="text-sm text-gray-500 mb-3">Loading cities...</div>
            )}
            {!loadingCities && !formData.cityId && (
              <div className="mb-3 p-2.5 bg-red-50 rounded-lg border border-red-200">
                <div className="flex items-start">
                  <AlertCircle className="h-3.5 w-3.5 text-red-600 mr-2 mt-0.5 flex-shrink-0" />
