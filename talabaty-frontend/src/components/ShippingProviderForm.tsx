...
export default function ShippingProviderForm({ storeId, providerName, providerType, existingProvider, onClose, onSuccess, }: ShippingProviderFormProps) {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.log(isEditMode ? 'Updated provider:' : 'Created provider:', provider);
            onSuccess();
        }
        catch (err: any) {
      ...
      <div className="relative top-20 mx-auto p-5 border w-full max-w-md shadow-lg rounded-md bg-white">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            {logo ? (<img src={logo} alt={title} className="h-10 w-10 object-contain" onError={(e) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input id="displayName" name="displayName" type="text" value={formData.displayName} onChange={handleChange} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm"/>
          </div>
          ...
          <div>
            ...
            <input id="customerId" name="customerId" type="text" required value={formData.customerId} onChange={handleChange} placeholder={`Enter ${providerConfig?.displayName || 'Provider'} Customer ID`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm" style={{
          ...
          <div>
            ...
            <input id="apiKey" name="apiKey" type="password" required={!isEditMode} value={formData.apiKey} onChange={handleChange} placeholder={isEditMode ? "Leave blank to keep existing API key" : `Enter ${providerConfig?.displayName || 'Provider'} API Key`} className="block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm" style={{
            '--tw-ring-color': storeColor,
        } as React.CSSProperties}/>
            <p className="mt-1 text-xs text-gray-500">
              {isEditMode
          ...
          <div className="flex items-center justify-end space-x-3 pt-4">
            ...
            <button type="submit" disabled={loading} className="px-4 py-2 text-sm font-medium text-white rounded-md hover:opacity-90 disabled:opacity-50 transition-opacity" style={{ backgroundColor: storeColor }}>
