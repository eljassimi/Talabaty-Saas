    ...
    return (<div className="space-y-6">
      <div className={hideTitle ? 'p-6' : 'bg-white rounded-lg shadow-sm border border-gray-200 p-6'}>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {Object.values(DELIVERY_PROVIDERS).map((provider) => (<div key={provider.name} className="border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer" onClick={() => {
                if (stores.length > 0) {
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  {provider.logo ? (<img src={provider.logo} alt={provider.name} className="h-12 w-12 object-contain" onError={(e) => {
                ...
                <button className="p-2 text-gray-400 hover:text-gray-600 transition-colors" style={{ color: storeColor }}>
      ...
      {Object.keys(storeProviders).length > 0 && (<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            {Object.entries(storeProviders).map(([storeId, providers]) => {
                ...
                if (!providers.length)
                    return null;
                return (<div key={storeId} className="border border-gray-200 rounded-lg p-4">
                  ...
                  <div className="space-y-2">
                    {providers.map((provider) => {
                        ...
                        return (<div key={provider.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                          <div className="flex items-center space-x-3">
                            {providerLogo ? (<img src={providerLogo} alt={provider.displayName} className="h-8 w-8 object-contain" onError={(e) => {
                                    e.currentTarget.style.display = 'none';
                                    const fallback = e.currentTarget.nextElementSibling as HTMLElement;
                                    if (fallback)
                                        fallback.style.display = 'flex';
                                }}/>) : null}
                            <div className="h-8 w-8 rounded flex items-center justify-center" style={{
                                backgroundColor: storeColor + '20',
                          ...
                          <button onClick={() => {
                                ...
                                setShowProviderForm(true);
                            }} className="text-sm font-medium hover:opacity-80 transition-opacity px-3 py-1 rounded-md" style={{
                                color: storeColor,
      ...
      {showProviderForm && selectedStore && (<ShippingProviderForm storeId={selectedStore} providerName={selectedProvider || undefined} existingProvider={editingProvider || undefined} onClose={() => {
