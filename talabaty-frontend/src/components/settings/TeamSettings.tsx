    ...
    return (<div className="space-y-6">
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="relative">
          ...
          <input type="email" placeholder="Enter email address, press enter to add.." value={emailInput} onChange={(e) => setEmailInput(e.target.value)} onKeyDown={handleAddEmail} className="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent transition-all" style={{
        ...
        {emails.length > 0 && (<div className="mt-3 flex flex-wrap gap-2">
            {emails.map((email) => (<span key={email} className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-blue-100 text-blue-800">
                {email}
                <button onClick={() => handleRemoveEmail(email)} className="ml-2 transition-colors" style={{ color: '#123133' }} onMouseEnter={(e) => e.currentTarget.style.color = '#FF6E00'} onMouseLeave={(e) => e.currentTarget.style.color = '#123133'}>
                  ×
                </button>
              </span>))}
          </div>)}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={() => setShowCreateModal(true)} className="flex items-center px-4 py-2 text-white rounded-lg hover:opacity-90 transition-all shadow-sm" style={{ backgroundColor: storeColor }}>
        ...
        {members.length === 0 ? (<div className="text-center py-12">
            ...
            {members.map((member) => {
                ...
                return (<div key={member.id} className="flex items-center justify-between p-4 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                  <div className="flex items-center space-x-3 flex-1">
                    ...
                    <div className="flex items-center space-x-2">
                      ...
                      <button onClick={() => handleRemoveMember(member.id)} disabled={removingMemberId === member.id} className="p-2 text-red-600 hover:bg-red-50 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed" title="Remove team member">
                        {removingMemberId === member.id ? (<div className="animate-spin rounded-full h-4 w-4 border-b-2 border-red-600"></div>) : (<Trash2 className="h-4 w-4"/>)}
                      </button>
                    </div>
                  </div>
                </div>);
      ...
      {showCreateModal && user?.selectedStoreId && (<CreateTeamMemberModal storeId={user.selectedStoreId} onClose={() => {
