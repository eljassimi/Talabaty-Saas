      ...
      if (status === 'CONFIRMED' && order.status !== 'CONFIRMED') {
        try {
          ...
          if (!order.city || order.city === 'N/A' || !cityExistsInDeliveryPlatform(order.city)) {
            // City not found or missing - silently skip shipping, warning icon will show in table
          ...
          // Silently fail - warning icon in table will indicate issues
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent transition-all resize-none"
              style={{
                '--tw-ring-color': storeColor,
              } as React.CSSProperties & { '--tw-ring-color': string }}
              onFocus={(e) => {
