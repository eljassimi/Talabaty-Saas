...
import { LayoutDashboard, Package, Store, Plug2, Users, MessageCircle, HelpCircle, Settings, ChevronLeft, Banknote, Wallet, type LucideIcon, } from 'lucide-react';
...
export default function Sidebar({ open, onClose, navItems: navItemsProp }: SidebarProps) {
    ...
    return (<>
      ...
      {open && (<div className="fixed inset-0 bg-black/30 z-40 lg:hidden transition-opacity dark:bg-black/50" onClick={onClose} aria-hidden="true"/>)}
      ...
      <aside className={`
        ...
        <div className="flex items-center gap-3 h-16 px-5 border-b shrink-0" style={{ borderColor: sidebarBorder }}>
          <button type="button" onClick={onClose} className="lg:hidden p-1.5 rounded-lg text-[#6B7280] dark:text-gray-400 hover:bg-[#F6F8FB] dark:hover:bg-[#2A2D35] hover:text-[#111827] dark:hover:text-gray-100" aria-label="Close menu">
            <ChevronLeft className="h-5 w-5"/>
          </button>
          {storeLogo ? (<img src={storeLogo} alt={storeName || 'Store'} className="h-9 w-9 rounded-lg object-cover border border-[#E6E8EC] dark:border-gray-600"/>) : (<div className="h-9 w-9 rounded-lg flex items-center justify-center shrink-0" style={{ backgroundColor: storeColor }}>
              <Store className="h-5 w-5 text-white"/>
            </div>)}
          <span className="font-semibold truncate flex-1" style={{ color: isDark ? '#f3f4f6' : '#111827' }}>
            {storeName || 'Store'}
          </span>
        </div>
            ...
            return (<Link key={item.href} to={item.href} onClick={onClose} className={`
                ...
                `} style={active && !isDark
                    ...
                    : active && isDark
                        ...
                        : undefined}>
                <Icon className="h-5 w-5 shrink-0" style={active ? (isDark ? { color: '#ffffff' } : { color: storeColor }) : undefined}/>
                <span className="truncate">{item.name}</span>
                {item.badge != null && (<span className="ml-auto text-xs font-medium text-[#6B7280] dark:text-gray-400 bg-[#E6E8EC] dark:bg-[#2A2D35] px-2 py-0.5 rounded-md">
                    {item.badge}
                  </span>)}
              </Link>);
        })}
        ...
        <div className="p-3 border-t shrink-0 space-y-1" style={{ borderColor: sidebarBorder }}>
          <Link to="/help" onClick={onClose} className={`
            ...
            : undefined}>
            <HelpCircle className="h-5 w-5 shrink-0" style={location.pathname.startsWith('/help') ? (isDark ? { color: '#ffffff' } : { color: storeColor }) : undefined}/>
            <span>Help & Support</span>
          </Link>
          <Link to="/settings" onClick={onClose} className={`
              flex items-center gap-3 px-3 py-2.5 rounded-r-xl rounded-l-md text-sm font-medium
              transition-colors duration-200
            ...
            ? isDark
                ...
                : { backgroundColor: `${storeColor}22`, color: storeColor }
            : undefined}>
            <Settings className="h-5 w-5 shrink-0" style={location.pathname.startsWith('/settings') ? (isDark ? { color: '#ffffff' } : { color: storeColor }) : undefined}/>
            <span>Settings</span>
          </Link>
        </div>
      </aside>
