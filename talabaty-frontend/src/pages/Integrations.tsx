...
export default function Integrations() {
    ...
    const getEcomStatus = (id: string) => {
        if (id === 'youcan')
            return youcanConnected;
        return false;
    };
    const handleEcomClick = (id: string) => {
        if (id === 'shopify' || id === 'youcan')
            return;
    };
    if (!user?.selectedStoreId) {
    ...
    return (<div className="space-y-8">
      ...
      <section className="bg-white rounded-xl border border-[#E6E8EC] p-6 shadow-sm">
        ...
        <p className="text-sm text-[#6B7280] mb-6">
          Connect where your orders come from. Sync orders automatically from YouCan.
        </p>
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {ECOM_PLATFORMS.map((platform) => {
            ...
            return (<div key={platform.id} onClick={() => !platform.comingSoon && (isYouCan ? undefined : handleEcomClick(platform.id))} className={`
                ...
                <div className="flex items-start justify-between">
                  <div className="flex items-center gap-3 min-w-0">
                    {platform.logoUrl ? (<div className="h-12 w-12 rounded-xl flex items-center justify-center bg-white border border-[#E6E8EC] overflow-hidden p-1.5 shrink-0">
                        <img src={platform.logoUrl} alt={platform.name} className="h-full w-full object-contain"/>
                      </div>) : (<div className="h-12 w-12 rounded-xl flex items-center justify-center text-white font-bold text-lg shrink-0" style={{ backgroundColor: platform.iconBg }}>
                        {platform.icon}
                      </div>)}
                  ...
                  {connected && (<div className="flex items-center gap-1.5 shrink-0" style={{ color: storeColor }}>
                      <Check className="h-4 w-4"/>
                      <span className="text-xs font-medium">Connected</span>
                    </div>)}
                  {platform.comingSoon && (<span className="text-xs font-medium text-[#6B7280] bg-[#E6E8EC] px-2 py-1 rounded-md shrink-0">
                      Coming soon
                    </span>)}
                </div>
                {connected && isYouCan && (<div className="mt-4 pt-4 border-t border-[#E6E8EC] flex flex-wrap items-center gap-2">
                    <button type="button" onClick={(e) => { e.stopPropagation(); handleDisconnectYouCan(); }} disabled={disconnectingYouCan} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg disabled:opacity-50">
        ...
        {!youcanConnected && (<div className="mt-6 pt-6 border-t border-[#E6E8EC]">
