...
export default function SelectStore() {
    ...
    if (loading) {
        return (<div className="min-h-screen flex items-center justify-center bg-white dark:bg-[#222328]">
    ...
    return (<div className="relative min-h-screen bg-white dark:bg-[#222328] py-12 px-4 sm:px-6 lg:px-8">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="flex justify-center mb-4">
            <img src={TalabatyLogo} alt="Talabaty" className="h-10 w-auto"/>
          </div>
          ...
          <div className="mt-4">
            <button onClick={() => setShowCreateModal(true)} className="inline-flex items-center px-4 py-2.5 border border-transparent shadow-sm text-sm font-semibold rounded-lg text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-all" style={{
        ...
        {stores.length === 0 ? (<div className="bg-white dark:bg-[#2A2D35] rounded-xl shadow-sm dark:shadow-none border border-gray-200 dark:border-[#3d4048] p-12 text-center">
            ...
            {stores.map((store) => {
                ...
                return (<button key={store.id} onClick={() => !selecting && handleSelectStore(store.id)} disabled={selecting} className={`rounded-xl shadow-sm dark:shadow-none border-2 p-6 hover:shadow-lg dark:hover:shadow-none transition-all text-left relative group ${isSelected
                        ? 'border-green-500 bg-green-50 dark:bg-emerald-950/30'
                        : 'border-gray-200 dark:border-[#3d4048]'} ${selecting ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`} style={{
                        backgroundColor: isSelected ? undefined : theme === 'dark' ? '#2A2D35' : '#FFFFFF',
                        borderColor: isSelected ? storeColor : undefined,
                    }}>
                  <div className="flex items-center justify-between mb-4">
                    {store.logoUrl ? (<img src={store.logoUrl} alt={store.name} className="w-16 h-16 object-cover rounded-xl shadow-md border-2 border-gray-200 dark:border-[#3d4048]"/>) : (<div className="p-3 rounded-xl shadow-md" style={{ backgroundColor: storeColor }}>
                        <StoreIcon className="h-6 w-6 text-white"/>
                      </div>)}
                    {isSelected && (<div className="bg-green-500 p-2 rounded-full shadow-md">
                  ...
                  {isSelected && (<p className="mt-3 text-sm font-medium" style={{ color: storeColor }}>
                      Currently Selected
                    </p>)}
                </button>);
            })}
        ...
        {showCreateModal && (<CreateStoreModal onClose={() => setShowCreateModal(false)} onSuccess={async (newStore?: Store) => {
