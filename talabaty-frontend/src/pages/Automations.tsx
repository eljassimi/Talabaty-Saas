...
export default function Automations() {
    ...
    return (<div className="space-y-6">
      ...
      {error && (<div className="rounded-xl border border-red-200 dark:border-red-800 bg-red-50 dark:bg-red-900/20 p-4 text-red-700 dark:text-red-300 text-sm">
          {error}
        </div>)}
      {success && (<div className="rounded-xl border border-green-200 dark:border-green-800 bg-green-50 dark:bg-green-900/20 p-4 text-green-700 dark:text-green-300 text-sm">
          Settings saved.
        </div>)}

      <div className="rounded-xl border p-6 shadow-sm" style={{ backgroundColor: panelBg, borderColor: panelBorder }}>
        ...
        {settings?.sendingConfigured === false && (<div className="mb-4 p-4 rounded-xl border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20">
        ...
        {linkStatus?.provider === 'bridge' && (<div className="mb-6 p-6 rounded-xl border" style={{ backgroundColor: insetBg, borderColor: panelBorder }}>
            ...
            {linkStatus.ready ? (<p className="text-sm text-green-600 dark:text-green-400 flex items-center gap-2">
                <CheckCircle className="h-4 w-4"/>
                Your WhatsApp is linked. You can send automations and promotions to your customers.
              </p>) : linkStatus.qr ? (<>
                {linkStatus.reason === 'logged_out' && (<div className="mb-4 p-3 rounded-lg bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800">
                ...
                <div className="inline-block p-3 rounded-xl border" style={{ backgroundColor: inputBg, borderColor: panelBorder }}>
                  <img src={linkStatus.qr} alt="Scan this QR code with WhatsApp on your phone" className="w-[280px] h-[280px]"/>
                </div>
              </>) : linkStatus.error || linkStatus.bridgeError ? (<div className="rounded-lg border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 p-4">
              ...
              </div>) : linkStatus.initializing ? (<div className="flex items-center gap-3" style={{ color: textSecondary }}>
              ...
              </div>) : (<div className="flex items-center gap-3" style={{ color: textSecondary }}>
                <Loader2 className="h-5 w-5 animate-spin shrink-0" style={{ color: storeColor }}/>
                <div>
                  {linkStatus.reason === 'logged_out' ? (<>
                      <p className="text-sm font-medium">You were logged out. Preparing a new QR code…</p>
        ...
        <div className="flex items-center gap-3 mb-6">
          <button type="button" role="switch" aria-checked={settings?.whatsappAutomationEnabled ?? false} onClick={() => setSettings((s) => s ? { ...s, whatsappAutomationEnabled: !s.whatsappAutomationEnabled } : null)} className={`
            ...
            `} style={settings?.whatsappAutomationEnabled
        ...
        <div className="space-y-4">
          <div>
            ...
            <textarea value={settings?.whatsappTemplateConfirmed ?? ''} onChange={(e) => setSettings((s) => s ? { ...s, whatsappTemplateConfirmed: e.target.value } : null)} placeholder={DEFAULT_CONFIRMED} rows={5} className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-offset-0" style={{
          ...
          <div>
            ...
            <textarea value={settings?.whatsappTemplateDelivered ?? ''} onChange={(e) => setSettings((s) => s ? { ...s, whatsappTemplateDelivered: e.target.value } : null)} placeholder={DEFAULT_DELIVERED} rows={5} className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-offset-0" style={{
            backgroundColor: inputBg,
            borderColor: panelBorder,
            color: textPrimary,
            ['--tw-ring-color' as string]: storeColor,
        ...
        <div className="mt-4 p-3 rounded-lg border" style={{ backgroundColor: insetBg, borderColor: panelBorder }}>
        ...
        <div className="mt-6">
          <button type="button" onClick={handleSave} disabled={saving} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-50" style={{ backgroundColor: storeColor }}>
...
function PromoBroadcast({ storeId, storeColor, disabled, linkReady }: {
    storeId: string;
    storeColor: string;
    disabled: boolean;
    linkReady?: boolean;
    ...
    return (<div className="space-y-3">
      <textarea value={message} onChange={(e) => setMessage(e.target.value)} placeholder="e.g. Bonjour ! Nouvelle promo -20% ce week-end. Code: PROMO20" rows={4} className="w-full rounded-lg border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-offset-0" style={{
            backgroundColor: inputBg,
            borderColor: panelBorder,
            color: textPrimary,
            ['--tw-ring-color' as string]: storeColor,
      ...
      <div className="flex items-center gap-3">
        <button type="button" onClick={() => message.trim() && setConfirmOpen(true)} disabled={disabled || (linkReady === false) || !message.trim() || sending} className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white transition-opacity hover:opacity-90 disabled:opacity-50" style={{ backgroundColor: storeColor }}>
        ...
        {result && (<div className="flex flex-col gap-1">
            <span className="text-sm" style={{ color: textSecondary }}>
              ...
              {result.failed > 0 && (<span className="text-amber-600 dark:text-amber-400 ml-1">— {result.failed} failed</span>)}
            </span>
            {result.failedReasons && result.failedReasons.length > 0 && (<ul className="text-xs text-amber-700 dark:text-amber-300 mt-1 list-disc list-inside space-y-0.5">
                {result.failedReasons.slice(0, 5).map((reason, i) => (<li key={i}>{reason}</li>))}
                {result.failedReasons.length > 5 && (<li>… and {result.failedReasons.length - 5} more</li>)}
              </ul>)}
            {showNotReadyHint && (<div className="mt-2 p-3 rounded-lg bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-sm text-blue-800 dark:text-blue-200">
      ...
      {confirmOpen && (<div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50" onClick={() => setConfirmOpen(false)}>
          <div className="rounded-xl shadow-xl max-w-md w-full p-6 border" style={{ backgroundColor: isDark ? '#2A2D35' : '#FFFFFF', borderColor: panelBorder }} onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3 justify-end">
              <button type="button" onClick={() => setConfirmOpen(false)} className="px-4 py-2 rounded-lg text-sm font-medium hover:bg-gray-100 dark:hover:bg-[#3d4048]" style={{ color: isDark ? '#D1D5DB' : '#374151' }}>
                Cancel
              </button>
