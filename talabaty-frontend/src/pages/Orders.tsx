...
import { Plus, Package, Search, ChevronDown, Calendar, ChevronLeft, ChevronRight, AlertTriangle, RefreshCw, X, MessageCircle, Eye, Bike, Upload, CheckCircle, Clock, XCircle, FileText, Pencil } from 'lucide-react';
...
export default function Orders() {
    ...
    const effectiveStoreId = (user?.selectedStoreId && String(user.selectedStoreId).trim()) ||
    ...
    const loadOrdersForStore = useCallback(async (storeId: string) => {
        const loadId = ++latestLoadRef.current;
        setLoading(true);
        try {
            let ordersData: Order[] = selectedStatus === 'all'
                ? await orderService.getOrdersByStore(storeId)
                : await orderService.getOrdersByStoreAndStatus(storeId, selectedStatus as Order['status']);
            if (loadId !== latestLoadRef.current)
                return;
            const safeOrders = ordersData.filter((o) => o.storeId === storeId);
            setOrders(safeOrders);
            setLoadError(null);
            const ordersWithTracking = safeOrders.filter(o => o.ozonTrackingNumber && o.ozonTrackingNumber.trim() !== '');
            console.log(`Total orders loaded: ${safeOrders.length}, Orders with tracking numbers: ${ordersWithTracking.length}`);
            if (ordersWithTracking.length > 0) {
                console.log('Orders with tracking numbers:', ordersWithTracking.map(o => ({ id: o.id, trackingNumber: o.ozonTrackingNumber })));
            }
            loadTrackingStatuses(safeOrders);
        }
        catch (error: any) {
            if (loadId !== latestLoadRef.current)
                return;
    ...
    useEffect(() => {
        ...
        if (syncingYouCan) {
            const timer = setTimeout(() => {
                loadOrdersForStore(effectiveStoreId);
            }, 1500);
            return () => clearTimeout(timer);
        }
        loadOrdersForStore(effectiveStoreId);
    }, [effectiveStoreId, selectedStatus, syncingYouCan, loadOrdersForStore]);
    ...
    const syncYouCanStores = async () => {
        try {
            ...
            await Promise.allSettled(connectedStores.map(async (youcanStore) => {
                try {
    ...
    const getTrackingStatusBadge = (status: TrackingResponse | undefined, loading: boolean) => {
        ...
        return (<span className="text-xs font-medium px-2 py-0.5 rounded-full bg-gray-100 text-gray-800">
        {statusText}
      </span>);
    };
    ...
    if (loading && orders.length === 0) {
        return (<div className="flex flex-col items-center justify-center min-h-[320px] gap-4">
    ...
    return (<div className="space-y-8">
        ...
        <div>
          ...
          <p className="mt-3 text-gray-600">Manage all your orders.</p>
            ...
            <div className="flex items-center space-x-3">
              <button onClick={() => setShowBonDeLivraisonModal(true)} className="inline-flex items-center px-5 py-3 border border-gray-300 shadow-sm text-sm font-medium rounded-2xl text-gray-700 bg-white transition-all hover:bg-gray-50" title="Create Bon de Livraison (Ozon Express)">
                <FileText className="h-5 w-5 mr-2"/>
                Bon de Livraison
              </button>
              <button className="inline-flex items-center px-5 py-3 border border-gray-300 shadow-sm text-sm font-medium rounded-2xl text-gray-700 bg-white transition-all hover:bg-gray-50" title="Export orders">
                <Upload className="h-5 w-5 mr-2"/>
                Export
              </button>
              <button onClick={() => setShowCreateModal(true)} className="inline-flex items-center px-5 py-3 border border-transparent shadow-sm text-sm font-medium rounded-2xl text-white transition-all hover:shadow-lg" style={{ backgroundColor: storeColor }} onMouseEnter={(e) => {
      ...
      {loadError && (<div className="p-4 rounded-lg flex items-center justify-between bg-red-50 border border-red-200 text-red-700">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between space-y-4 md:space-y-0 md:space-x-4">
          ...
          <div className="flex-1 relative">
            ...
            <input type="text" placeholder="Search by product, customer name or phone" value={searchQuery} onChange={(e) => {
            setSearchQuery(e.target.value);
            setCurrentPage(1);
        }} className="w-full pl-10 pr-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:border-transparent transition-all" style={{
          ...
          <div className="flex items-center space-x-3">
            <div className="relative">
              <select value={selectedStatus} onChange={(e) => setSelectedStatus(e.target.value)} className="appearance-none bg-white border border-gray-300 rounded-lg px-4 py-2.5 pr-8 text-sm transition-all">
            ...
            <div className="relative">
              <select className="appearance-none bg-white border border-gray-300 rounded-lg px-4 py-2.5 pr-8 text-sm transition-all">
      ...
      {filteredOrders.length === 0 ? (<div className="bg-white rounded-2xl shadow-sm border border-gray-100 px-6 py-16 text-center">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                ...
                return (<tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900 max-w-[140px] truncate" title={order.customerPhone}>
                    ...
                    }} className="inline-flex items-center justify-center p-2.5 text-green-600 hover:bg-green-50 rounded-xl transition-colors" title="Open WhatsApp">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        <div className="flex items-center space-x-2">
                          ...
                          {(!order.city || order.city === 'N/A' || !cityExistsInDeliveryPlatform(order.city)) && (<div className="relative">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        ...
                        }} className="p-1 hover:bg-gray-100 rounded transition-colors" title="Refresh tracking status">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap">
                        {order.ozonTrackingNumber ? (<div className="flex flex-col items-center space-y-1.5">
                      ...
                      <td className="px-8 py-5 whitespace-nowrap text-center">
                        <button onClick={() => navigate(`/orders/${order.id}`)} className="inline-flex items-center justify-center p-2.5 text-gray-600 hover:bg-gray-100 rounded-xl transition-colors" title="View order details">
                    ...
                    }} className="p-2 hover:bg-gray-100 rounded-xl transition-colors" title="Edit status">
          ...
          <div className="flex items-center justify-between px-8 py-5 border-t border-gray-100 bg-white">
            ...
            <div className="flex items-center space-x-2">
              ...
              {totalPages > 1 && (<>
                  <button onClick={() => setCurrentPage(currentPage - 1)} disabled={currentPage === 1} className="p-1 border border-gray-300 rounded-md disabled:opacity-50 disabled:cursor-not-allowed">
                    <ChevronLeft className="h-4 w-4"/>
                  </button>
                  <span className="px-2 text-sm font-medium text-gray-900">{currentPage}</span>
                  <button onClick={() => setCurrentPage(currentPage + 1)} disabled={currentPage === totalPages} className="p-1 border border-gray-300 rounded-md disabled:opacity-50 disabled:cursor-not-allowed">
      ...
      {showCreateModal && (<CreateOrderModal stores={stores} onClose={() => setShowCreateModal(false)} onSuccess={() => {
                setShowCreateModal(false);
                if (effectiveStoreId)
                    loadOrdersForStore(effectiveStoreId);
            }}/>)}

      {showStatusModal && selectedOrder && (<UpdateOrderStatusModal order={selectedOrder} onClose={() => {
      ...
      {showBonDeLivraisonModal && (<CreateBonDeLivraisonModal orders={orders} onClose={() => setShowBonDeLivraisonModal(false)} onSuccess={() => effectiveStoreId && loadOrdersForStore(effectiveStoreId)} storeColor={storeColor}/>)}
