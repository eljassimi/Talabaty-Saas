...
export default function OrderDetail() {
    ...
    if (!order) {
        ...
        <Link to="/orders" className="mt-4 inline-flex items-center text-sm transition-colors" style={{ color: storeColor }}>
          Back to Orders
        </Link>
      </div>);
    }
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          {order.externalOrderId && (<p className="text-sm text-gray-500 mt-1">External ID: {order.externalOrderId}</p>)}
        </div>
        <div className="flex items-center space-x-3">
          <span className={`inline-flex px-4 py-2 text-sm font-semibold rounded-full border ${getStatusColor(order.status)}`}>
            {getStatusLabel(order.status)}
          </span>
          {canUpdateOrder && (<button onClick={() => setShowUpdateModal(true)} className="inline-flex items-center px-4 py-2 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white transition-all" style={{ backgroundColor: storeColor }} onMouseEnter={(e) => {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200" style={{ borderBottomColor: `${storeColor}30` }}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    {(!order.city || order.city === 'N/A' || !cityExistsInDeliveryPlatform(order.city)) && (<div className="group relative">
                        ...
                        <div className="absolute left-0 bottom-full mb-2 hidden group-hover:block z-50 w-80 p-4 bg-red-50 border-2 border-red-300 rounded-xl shadow-lg text-xs text-red-800 break-words">
                          ...
                          {!order.city || order.city === 'N/A' ? (<p className="mb-2">City is missing or set to "N/A". This order cannot be sent to the delivery platform.</p>) : (<p className="mb-2">City "<span className="font-semibold">{order.city}</span>" is not recognized in the delivery platform.</p>)}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="px-6 py-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {order.productName && (<div className="space-y-1">
                ...
                {order.productId && (<div className="space-y-1">
                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wide flex items-center">
                ...
                <div className="space-y-1 md:col-span-2">
                  ...
                  <div className="text-3xl font-bold px-4 py-3 rounded-lg" style={{
            color: storeColor,
            backgroundColor: `${storeColor}10`
                ...
                {order.ozonTrackingNumber && (<div className="space-y-1 md:col-span-2">
                    <label className="text-xs font-medium text-gray-500 uppercase tracking-wide flex items-center justify-between">
                      <div className="flex items-center">
                        <Truck className="h-3 w-3 mr-1"/>
                        Tracking Number
                      </div>
                      <button onClick={loadTrackingStatus} disabled={loadingTracking} className="p-1.5 hover:bg-gray-100 rounded-lg transition-colors disabled:opacity-50" title="Refresh tracking status">
                    ...
                    <div className="flex items-center space-x-3 bg-gray-50 px-4 py-3 rounded-lg border border-gray-200">
                      <img src="https://ozoneexpress.ma/wp/wp-content/uploads/2025/07/Untitled-design-38.png" alt="Ozon Express" className="h-8 w-8 object-contain" onError={(e) => {
                e.currentTarget.style.display = 'none';
                      ...
                      <div className="flex-1">
                        ...
                        {loadingTracking ? (<div className="flex items-center space-x-1.5 text-xs text-gray-500">
                            <RefreshCw className="h-3 w-3 animate-spin"/>
                            <span>Loading status...</span>
                          </div>) : trackingStatus ? (<div className="flex items-center space-x-2 mt-1">
                            {(() => {
                    ...
                    return (<span className="text-xs font-medium px-2 py-1 rounded-full bg-gray-100 text-gray-800">
                                  {statusText}
                                </span>);
                })()}
                          </div>) : (<span className="text-xs text-gray-400">Click refresh to check status</span>)}
                      </div>
                    </div>
                  </div>)}
                {order.assignedToName && (<div className="space-y-1 md:col-span-2">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="px-6 py-4 space-y-3">
              {order.status !== 'CONCLED' && (<button onClick={() => setShowShippingModal(true)} className="w-full inline-flex items-center justify-center px-4 py-2.5 border border-transparent shadow-sm text-sm font-medium rounded-lg text-white transition-all" style={{ backgroundColor: storeColor }} onMouseEnter={(e) => {
                const rgb = hexToRgb(storeColor);
                e.currentTarget.style.backgroundColor = `rgb(${Math.max(0, rgb.r - 20)}, ${Math.max(0, rgb.g - 20)}, ${Math.max(0, rgb.b - 20)})`;
            }} onMouseLeave={(e) => {
                e.currentTarget.style.backgroundColor = storeColor;
              ...
              {canUpdateOrder && (<div className="space-y-2 pt-2 border-t border-gray-200">
                  <label className="block text-sm font-medium text-gray-700">Update Status</label>
                  <select value={order.status} onChange={(e) => handleStatusUpdate(e.target.value as Order['status'])} className="block w-full px-3 py-2 border border-gray-300 rounded-lg shadow-sm focus:outline-none focus:ring-2 text-sm transition-all" style={{
                ...
                e.currentTarget.style.boxShadow = '';
                  ...
                  {statusUpdateError && (<div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg text-xs text-red-800">
      ...
      {showShippingModal && id && (<SendToShippingModal orderId={id} order={order} onClose={() => setShowShippingModal(false)} onSuccess={() => {
                setShowShippingModal(false);
                loadOrder();
            }}/>)}

      {showUpdateModal && order && (<UpdateOrderModal order={order} onClose={() => setShowUpdateModal(false)} onSuccess={() => {
