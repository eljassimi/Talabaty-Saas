...
export default function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const pending = filteredOrders.filter((o) =>
      ['ENCOURS', 'APPEL_1', 'APPEL_2'].includes(o.status)
    ).length
    const canceled = filteredOrders.filter((o) =>
  ...
  const trendNeutral: { value: string; direction: TrendDirection } = { value: '—', direction: 'neutral' }
  ...
  return (
    <div className="space-y-6">
      {/* Page title, filter, and primary action */}
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <Link
            to="/orders"
            className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-medium text-white transition-opacity hover:opacity-90 shrink-0"
      ...
      >
        <StatCard
          ...
          trend={trendNeutral}
          icon={<Package className="h-5 w-5" style={{ color: storeColor }} />}
        />
        <StatCard
          title="Confirmed"
        ...
        <StatCard
          ...
          trend={trendNeutral}
          icon={<TrendingUp className="h-5 w-5" style={{ color: storeColor }} />}
        />
        <StatCard
          title="Revenue"
