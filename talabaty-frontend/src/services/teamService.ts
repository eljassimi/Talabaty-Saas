...
export const teamService = {
    ...
    async bulkCreateTeamMembers(storeId: string, data: BulkCreateTeamRequest): Promise<BulkCreateTeamResponse> {
        const response = await api.post<BulkCreateTeamResponse>(`/stores/${storeId}/team/bulk-create`, data);
        return response.data;
    },
    async inviteUser(storeId: string, userId: string, role: TeamMember['role']): Promise<TeamMember> {
    ...
    async inviteExternalMember(storeId: string, email: string, role: TeamMember['role']): Promise<TeamMember> {
    ...
    async updateMemberRole(storeId: string, memberId: string, role: TeamMember['role']): Promise<TeamMember> {
        const response = await api.put<TeamMember>(`/stores/${storeId}/team/${memberId}/role`, {
            role,
        });
        return response.data;
    },
    async createTeamMember(storeId: string, data: {
        email: string;
        password?: string;
        firstName: string;
        lastName: string;
        ...
        const response = await api.post<{
            ...
            userWasCreated: boolean;
            generatedPassword?: string;
        }>(`/stores/${storeId}/team/create-member`, data);
        return response.data;
    },
};
