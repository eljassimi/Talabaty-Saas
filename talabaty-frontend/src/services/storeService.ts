...
export const storeService = {
    ...
    async createShippingProvider(storeId: string, data: CreateShippingProviderRequest): Promise<ShippingProvider> {
        const response = await api.post<ShippingProvider>(`/stores/${storeId}/shipping-providers`, data);
        return response.data;
    },
    ...
    async sendWhatsAppBroadcast(storeId: string, message: string): Promise<{
        total: number;
        sent: number;
        failed: number;
    ...
    async getSupportRevenueSettings(storeId: string): Promise<{
    ...
    async updateSupportRevenueSettings(storeId: string, data: {
        pricePerOrderConfirmedMad?: number;
        pricePerOrderDeliveredMad?: number;
    }): Promise<{
        pricePerOrderConfirmedMad: number;
        ...
        const response = await api.patch<{
            pricePerOrderConfirmedMad: number;
            pricePerOrderDeliveredMad: number;
        }>(`/stores/${storeId}/support-revenue-settings`, data);
        return response.data;
    },
};
